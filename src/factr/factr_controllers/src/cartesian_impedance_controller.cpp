...
// franka_example_controllers/src/fr3/cartesian_impedance_example_controller.cpp.
// Control law, gains, target filtering and null-space term are unchanged. Changes needed
// for franka_ros2 v0.1.0 / ROS 2 Humble / external targets:
//  - end-effector pose read from FrankaRobotModel::getPoseMatrix(kEndEffector) (O_T_EE)
//    instead of FrankaCartesianPoseInterface (needs franka_hardware's cartesian_pose_state
//    interfaces, which v0.1.0 does not export); arm_prefix parameter dropped with it.
//  - the built-in demo motion (updateMotionTarget) removed: it overwrote the target every
//    cycle, so poses published on ~/equilibrium_pose were ignored.
//  - ~/set_cartesian_stiffness service removed (franka_msgs/SetCartesianStiffness does not
//    exist in v0.1.0); stiffness is set via the translational/rotational_stiffness params.
//  - Humble hardware_interface API: get_value() / void set_value().
//  - arm_id defaults to "panda" (v0.1.0 joint/interface names).

#include "factr_controllers/cartesian_impedance_controller.hpp"

#include <cassert>

namespace factr_controllers {

CartesianImpedanceController::CartesianGains CartesianImpedanceController::buildGains(
...
CartesianImpedanceController::state_interface_configuration() const {
  ...
  // 2) Robot model + state for pose, coriolis, jacobian.
...
CartesianImpedanceController::CallbackReturn CartesianImpedanceController::on_configure(
  ...
  param_cb_handle_ = get_node()->add_on_set_parameters_callback(
      std::bind(&CartesianImpedanceController::onParameterUpdate, this, std::placeholders::_1));
...
CartesianImpedanceController::CallbackReturn CartesianImpedanceController::on_activate(
  ...
  std::tie(orientation_init, position_init) = getCurrentOrientationAndTranslation();
...
CartesianImpedanceController::getCurrentOrientationAndTranslation() {
  const std::array<double, 16> pose_array =
      franka_robot_model_->getPoseMatrix(franka::Frame::kEndEffector);
  const Eigen::Affine3d pose(Eigen::Matrix4d::Map(pose_array.data()));  // column-major
  return {Eigen::Quaterniond(pose.rotation()), pose.translation()};
}
...
controller_interface::return_type CartesianImpedanceController::update(
  ...
  std::tie(orientation, position) = getCurrentOrientationAndTranslation();
...
Eigen::Matrix<double, 6, 1> CartesianImpedanceController::computeError(
...
rcl_interfaces::msg::SetParametersResult CartesianImpedanceController::onParameterUpdate(
...
PLUGINLIB_EXPORT_CLASS(factr_controllers::CartesianImpedanceController,
