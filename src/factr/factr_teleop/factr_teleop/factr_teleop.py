...
class FACTRTeleop(Node, ABC):
    ...
    def __init__(self):
        ...
        self.torque_opt_damping = self.config["controller"]["null_space_torque_optimization"][
                "damping"
        ]
    ...
    def _prepare_dynamixel(self):
        ...
        servo_port_index = np.array(
            self.config["dynamixel"]["servo_port_index"], dtype=int
        )
        assert (
            self.num_motors == len(self.joint_signs) == len(servo_port_index)
        ...
        for port_i, port_name in enumerate(self.config["dynamixel"]["ports"]):
            ...
            try:
                driver = DynamixelDriver(
                    joint_ids[motor_indices],
                    [self.servo_types[i] for i in motor_indices],
                    dynamixel_port,
                )
            except FileNotFoundError:
                self.get_logger().info(
                    f"Port {dynamixel_port} not found. Please check the connection."
                )
                return
    ...
    def _prepare_inverse_dynamics(self):
        ...
        self.pin_model, _, _ = pin.buildModelsFromUrdf(
            filename=urdf_model_path, package_dirs=urdf_model_dir
        )
    ...
    def _match_start_pos(self):
        ...
        print(f"init_pos: {self.initial_match_joint_pos[0 : self.num_arm_joints]}")
        while (
            np.linalg.norm(
    ...
    def objective_torque_opt(self, q, K, o_T_eef):
